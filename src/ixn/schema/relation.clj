...
(def City [:map {:closed? true? :title {:en "city" :nl "plaats"}}
           ...
           [:city/country {:title {:en "country" :nl "land"}} Country]])
...
(def Address [:map {:closed? true :title {:en "address" :nl "adres"}}
              ...
              [:address/lat-lon {:title {:en "coordinates" :nl "coördinaten"}}
               [:map
...
(def LegalForm [:enum {:title {:en "legal form" :nl "rechtsvorm"}} :eenmanszaak :vof :stichting :bv :cv :nv])

(def Person
  [:map {:closed true :title {:en "person" :nl "persoon"}}
   ...
   [:relation/addresses [:repeat {:min 1 :max 1} Address]]
   [:relation/communication [:repeat {:min 1 :max 5} Communication]]])

(def Organisation
  [:map {:closed true :title {:en "organisation" :nl "organisatie"}}
   [:relation/id {:title {:en "id" :nl "id"}} uuid?]
   [:relation/name {:title {:en "name" :nl "naam"}} string?]
   [:relation/legal-form {:title {:en "legal form" :nl "rechtsvorm"}} LegalForm]
   [:relation/bsn {:title {:en "bsn" :nl "bsn"}} string?]
...
(comment
  (mg/generate Person)
  (mg/generate Organisation)
  (mg/generate LegalForm)
  (:relation/legal-form (mg/generate Organisation))

  (map (fn [_] (:relation/registration_id (mg/generate Organisation))) (range 10))
  ...)
